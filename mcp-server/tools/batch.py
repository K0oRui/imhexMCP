...
TOOLS = [
    Tool(
        name="batch_open_directory",
        description="Open multiple binary files from a directory for batch analysis",
        inputSchema={
            "type": "object",
            "properties": {
                "directory": {
            ...
            "properties": {
                "patterns": {
                    ...
                    "items": {
                        "type": "object",
                        "properties": {
                            "value": {"type": "string", "description": "Pattern value"},
                            "type": {
                                "type": "string",
                                "enum": ["hex", "string"],
                ...
                "provider_ids": {
                    "description": "Provider IDs or 'all'",
                    "oneOf": [
    ...
    Tool(
        name="batch_hash",
        description="Calculate hashes for multiple files simultaneously",
        inputSchema={
            "type": "object",
            "properties": {
                "algorithms": {
    ...
    Tool(
        name="batch_diff",
        description="Compare a reference file against multiple targets",
        inputSchema={
            "type": "object",
            "properties": {
                "reference_id": {
                    "type": "integer",
                    "description": "Reference provider ID",
                    "minimum": 0,
                },
                "target_ids": {
                    "description": "Target IDs or 'all'",
                    "oneOf": [
                ...
                "max_diff_regions": {
                    "type": "integer",
                    "description": "Max diff regions per file",
                },
            },
    ...
    Tool(
        name="batch_search_single",
        description="Search a single pattern across all open files",
        inputSchema={
            "type": "object",
            "properties": {
                "provider_ids": {
                    "description": "Provider IDs or 'all' (default: all)",
                    "oneOf": [
                        {"type": "array", "items": {"type": "integer"}},
                        {"type": "string", "enum": ["all"]},
                    ],
    ...
    Tool(
        name="batch_hash_range",
        description="Hash files with offset/size range control",
        inputSchema={
            "type": "object",
            "properties": {
                "provider_ids": {
...
def handle_batch_open_directory(client: ImHexClient, args: dict) -> str:
    ...
    files = sorted(
        [
            ...
            for p in d.iterdir()
            if p.is_file()
            and p.suffix.lower() in (".dll", ".exe", ".bin", ".so", ".elf")
        ]
    )[:max_files]
    if not files:
    ...
    lines.insert(
        0,
        f"Files found: {len(files)}  Opened: {opened}  Skipped: {len(files) - opened}  Total providers: {len(files_now)}",
    )
    for e in errors[:5]:
...
def handle_batch_search(client: ImHexClient, args: dict) -> str:
    ...
    cmd_args = {
        "pattern": val,
        "provider_ids": args.get("provider_ids", "all"),
        "max_matches_per_file": args.get("max_matches_per_file", 1000),
    }
    d = client.send_command("batch/search", cmd_args).get("data", {})
    results = d.get("results", [])
    lines = [
        f"Pattern: {val}  Total matches: {d.get('total_matches', 0)}  Files: {d.get('total_files', 0)}"
    ]
    for r in results:
...
def handle_batch_diff(client: ImHexClient, args: dict) -> str:
    ...
    s = d.get("summary", {})
    lines = [
        f"Reference: {s.get('reference_file', '?')}  Files compared: {s.get('files_compared', 0)}  Avg similarity: {s.get('avg_similarity', 0):.2f}%"
    ]
...
def handle_batch_search_single(client: ImHexClient, args: dict) -> str:
    ...
    results = d.get("results", [])
    lines = [
        f"Pattern: {args.get('pattern', '')}  Max per file: {args.get('max_matches', 1000)}"
    ]
    total = 0
    for r in results:
        matches = r.get("matches", [])
